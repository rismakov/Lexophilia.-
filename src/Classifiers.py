...
class Classifiers(object):
    ...
    def cross_validate(self, X, y):
        params_list = [{'n_estimators':[60,70,80,90,100], 'learning_rate': [0.01,0.05,0.1]} ,\
                        {'n_estimators':[50,60,70,90,100,110,120], 'learning_rate': [0.05,0.5,1], 'max_depth':[1,3,5,10]},\
                        {'n_estimators':[100,500],'max_depth':[20,50],'criterion':['gini']}]
        for clf,params in zip(self.classifiers[2],params_list[2]):
            ...
            scores = cross_val_score(clf, self._X_train, self._y_train, cv=5, scoring='accuracy')
