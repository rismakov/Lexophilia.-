...
def get_df_and_grouped_df_all_media(media_sites,filter_date=False,min_date=None):
    '''
    ...
    for media_name in media_sites:
        data,grouped_data = get_df_and_grouped_df(media_name,filter_date=filter_date,min_date=min_date)
...
if __name__ == "__main__":
    ...
    media_sites = ['TIME', 'TIME_Opinion','Atlantic','Buzzfeed','Breitbart','Slate']
    #media_sites = ['Slate','nyt']

    media_data, media_grouped_data = get_df_and_grouped_df_all_media(media_sites,filter_date=True,min_date=2011)

    #combined_data = combine_data(media_grouped_data)


    '''
    run_test(media_data[0], 'slate2.jpg')
    run_test(media_data[1], 'nyt.jpg')
    run_test(combined_data, 'nyt_and_slate.jpg')
    '''
    ...
    #Plotting.plot_features_by_gender(combined_grouped_data,'images-gender/{}_by_gender.jpg')
