...
def print_gender_stats(df,female_df, male_df, feature):
    ...
    if p_value < 0.05:
        if female_mean > male_mean:
            print 'Female significantly larger with p value of {}'.format(p_value)
        elif male_mean > female_mean:
            print 'Male significantly larger with p value of {}'.format(p_value)
...
def create_errorbar_graph_of_mean_feature_values(means,errs,names,feature,colors):
    for i,mean,err,color in izip(xrange(len(means)),means,errs,colors):
        plt.errorbar(i,mean, yerr = err, fmt ='o',color=color)
    plt.ylabel(YLABELS.get(feature,feature))
...
def plot_feature_vs_date(media_data,media_names,filename,colors):
    ...
    for feature in media_data[0].columns:
        if isinstance(media_data[0].iloc[0][feature],int) or isinstance(media_data[0].iloc[0][feature],float):
            means=[]
            errs=[]
            ...
            plt.subplot(1,2,2)
            create_errorbar_graph_of_mean_feature_values(means,errs,media_names,feature,colors)
...
def plot_top_countries_for_each_newssite(media_data,media_sites):
    ...
    plt.style.use('fivethirtyeight')
    plt.savefig('images/top_countries.jpg',facecolor='whitesmoke')
    plt.close()
...
def plot_gender_vs_time(data):
    female_data, male_data = seperate_by_gender(data)
